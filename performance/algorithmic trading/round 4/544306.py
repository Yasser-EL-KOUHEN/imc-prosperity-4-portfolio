...
# ----------------------------------------------------------------------
#  Counterparty flow scan (Round 4)
# ----------------------------------------------------------------------

def scan_counterparty_flow(state: "TradingState") -> dict:
    """Scan VELVETFRUIT_EXTRACT counterparty trades from BOTH sources (one-tick lagged):
      - state.market_trades: trades between other bots that did NOT involve our algo
      - state.own_trades:    trades where our algo was the counterparty

    Both must be scanned. When Mark 67 takes our passive ask, or Mark 49 fills our bid,
    those appear ONLY in own_trades -- missing them would undercount events by up to 100%.

    Mark 67: systematic dip buyer (+bullish). 46-61 events/day.
    Mark 49: passive high-seller (-bearish, local high confirmed). 32-40 events/day.
    Mark 22: aggressive VEL seller (tracked separately; too frequent for composite).
    """
    mark67_events = 0
    mark49_events = 0
    mark22_sell_events = 0
    for source in (state.market_trades, state.own_trades):
        for trade in source.get("VELVETFRUIT_EXTRACT", []):
            ...
            if trade.seller == "Mark 22":
                mark22_sell_events += 1
    return {
        "mark67_events": mark67_events,
        "mark49_events": mark49_events,
        "mark22_sell_events": mark22_sell_events,
    }
...
class VoucherSurface:
    ...
    # Competition-specified TTE at the start of each backtester day.
    # Round 4 uses 1-indexed day numbers (1, 2, 3).
    DAY_TTE: Dict[int, float] = {1: 7.0, 2: 6.0, 3: 5.0}
    ...
    def calibrate_tte(self, spot: float, order_depths: Dict) -> None:
        ...
        if self._tte_calibrated or spot <= 0:
            return
        day_num = min(self._day_count + 1, 3)  # Round 4 days are 1-indexed
        self.initial_tte_days = self.DAY_TTE.get(day_num, 5.0)
        self._day_count += 1
...
class VEVUnderlying:
    ...
    def orders(self, state: TradingState, snapshot: dict,
               net_delta: float, mark_net: int = 0,
               mark49_last_ts: int = -10000) -> Tuple[List[Order], float]:
        ...
        if spread >= 5:
            ...
            if passive_bid < passive_ask:
                ...
                # Composite counterparty tilt (session-level regime, not per-tick).
                # mark_net = mark67_ticks - mark49_ticks accumulated this day.
                # Symmetric frequencies (~50 vs ~36 events/day) keep the score balanced.
                if mark_net >= 5:    # Mark 67 dominant → bullish regime
                    bid_extra += 3
                elif mark_net <= -3: # Mark 49 dominant → bearish regime, trim bid
                    bid_extra = max(0, bid_extra - 2)
                bid_size = min(buy_cap, 12 + bid_extra)
                ask_size = min(sell_cap, 12 + ask_extra)
                # Signal C: Mark 49 confirms local high at prior tick.
                # Halve bid size for 500ms to avoid buying into the high's tail.
...
class VoucherTrader:
    ...
    def orders(self, state: TradingState, spot_fair: float
        ...
        for product, config in self.ACTIVE_CONFIG.items():
            ...
            # Note: VEV_6000/6500 typically have bid=0, ask=1. Selling at ask=1 never
            # fills (Mark 22 supplies at 0), but selling at bid=0 causes daily MTM losses
            # since the backtester settles at BS fair value each day. Keep unfilled = 0 PnL.
...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        # --- Counterparty flow scan (Signals B + C) ---
        # Scan runs only on non-day-start ticks: state.market_trades at timestamp==0
        # contains prior-day last-tick data (one-tick lag crosses day boundary), so
        # scanning there would contaminate mark_net and falsely trigger Signal C.
        if state.timestamp == 0:
            mark_net = 0          # reset composite score at day start
            mark49_last_ts = -10000
        else:
            mark_net = saved.get("mark_net", 0)
            mark49_last_ts = saved.get("mark49_last_ts", -10000)
            flow = scan_counterparty_flow(state)
            # Composite: +1 per Mark 67 buy tick (bullish dip-buyer),
            #            -1 per Mark 49 sell tick (bearish local-high confirmed).
            # Mark 22 (~184 sells/day) excluded — frequency mismatch would dominate.
            if flow["mark67_events"] > 0:
                mark_net += 1
            if flow["mark49_events"] > 0:
                mark_net -= 1
                mark49_last_ts = state.timestamp  # detection time, not trade CSV timestamp
        # ------------------------------------------------
        ...
        if vev_snapshot is not None:
            ...
            vev_orders, _ = self.vev.orders(
                ...
                mark_net=mark_net, mark49_last_ts=mark49_last_ts,
            )
            if vev_orders:
