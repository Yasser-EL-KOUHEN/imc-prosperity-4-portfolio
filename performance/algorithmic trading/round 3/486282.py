...
class HydrogelMM:
    ...
    def __init__(self):
        # v6 hardcoded values from research/round3/hydrogel_sweep.py winner.
        # For re-tuning, edit values here directly.
        self.take_threshold = 2.5
        self.passive_edge = 1.2
        # OBI disabled (v7 backtests confirmed it's net-negative).
        self.obi_beta = 0.0
...
class VoucherSurface:
    """Per-strike sigma calibrated to historical means.  See
    research/round3/iv_surface.py per_strike_iv_summary.csv."""
    ...
    STRIKE_SIGMA = {
        ...
        5200: 0.244,
        5300: 0.245,
        5400: 0.230,   # structural dent (-1.5 vol pts)
        5500: 0.249,
        6000: 0.240,
        6500: 0.240,
    }
    ...
    def __init__(self):
        self.initial_tte_days = 5.0
        self._tte_calibrated = False

    def calibrate_tte(self, spot: float, order_depths: Dict) -> None:
        """One-time TTE calibration from first-tick market prices.
        Finds T s.t. BS(spot, K, T/365, sigma) = observed mid for the
        most liquid near-ATM voucher.  Works for any day (historical
        TTE=8/7/6 or live TTE=5/4/...) without os.environ.
        """
        if self._tte_calibrated or spot <= 0:
            return
        for strike in [5400, 5300, 5200, 5500, 5000, 5100, 4500, 6000]:
            product = f"VEV_{strike}"
            od = order_depths.get(product)
            if od is None or not od.buy_orders or not od.sell_orders:
                continue
            market_mid = (max(od.buy_orders.keys()) + min(od.sell_orders.keys())) / 2.0
            if market_mid <= 0.0:
                continue
            sigma = self.STRIKE_SIGMA.get(strike, 0.243)
            lo, hi = 0.2, 20.0
            for _ in range(60):
                t = (lo + hi) / 2.0
                price = black_scholes_call(spot, float(strike), t / 365.0, sigma)
                if price < market_mid:
                    lo = t
                else:
                    hi = t
            tte = (lo + hi) / 2.0
            if 0.5 <= tte <= 15.0:
                self.initial_tte_days = tte
                self._tte_calibrated = True
                return

    def get_state(self) -> dict:
        return {"tte": self.initial_tte_days, "cal": self._tte_calibrated}

    def set_state(self, d: dict) -> None:
        if d:
            self.initial_tte_days = float(d.get("tte", 5.0))
    ...
    def fair_and_delta(self, symbol: str, spot_fair: float, timestamp: int) -> Tuple[float, float]:
        ...
        sigma = self.STRIKE_SIGMA.get(strike, 0.24)
...
class IVMonitor:
    ...
    def update(self, symbol: str, market_mid: float, spot: float, timestamp: int) -> Optional[float]:
        ...
        sigma_init = self.last_sigma.get(strike, self.surface.STRIKE_SIGMA.get(strike, 0.24))
...
class GreeksLedger:
    ...
    def from_state(cls, state: TradingState, surface: "VoucherSurface",
        ...
        for strike, sigma in surface.STRIKE_SIGMA.items():
            product = f"VEV_{strike}"
...
class VEVUnderlying:
    ...
    def orders(self, state: TradingState, snapshot: dict,
        ...
        if abs(net_delta) > GreeksLedger.DELTA_CAP_HARD and spread > 0:
...
class VoucherTrader:
    ...
    def orders(self, state: TradingState, spot_fair: float
        ...
        for product, config in self.ACTIVE_CONFIG.items():
            ...
            allow_sell = config["allow_naked_short"] or pos > 0 or self.ledger.delta > 50.0
            if allow_sell:
                for bid_price in sorted(order_depth.buy_orders.keys(), reverse=True):
                    cap = _sell_cap()
                    if cap <= 0:
            ...
            # 2. Passive quotes inside spread, biased by z-score
            if spread >= 2:
                passive_bid = best_bid + 1
                passive_ask = best_ask - 1
                ...
                should_quote_ask = config["two_sided"] or pos > 0 or self.ledger.delta > 40.0
